function [acc_per_image, acc_per_class] = func_recognition(trainFeatures,testFeatures,trainLabels,testLabels,classifierType)
...
% calculate ACC
acc_per_image = sum(predLabels==testLabels)/length(testLabels);
for i = 1:num_class
    acc_per_class(i) = sum((predLabels == testLabels).*(testLabels==i))/sum(testLabels==i);
end
fprintf('Acc per image:%0.3f,Mean acc per class: %0.3f\n', acc_per_image, mean(acc_per_class));
