...
outfilename = ['./results_semi_HDA/officeHome-resnet2vgg-' classifierType '-sp-' num2str(using_sp) '-alpha-' num2str(alpha) '-lambda' num2str(lambda)  '-deltaS-' num2str(deltaS) '-deltaT-' num2str(deltaT)  '-PcaDim-' num2str(pcaDim) '-LppDim-' num2str(lppDim) '-T-' num2str(T)]
...
for randseed = 1:10
    for source_domain_index = 1:length(domains)
        ...
        for target_domain_index = 1:length(domains)
            ...
            if using_sp==1
                [acc_per_image{source_domain_index}{target_domain_index}{randseed},acc_per_class{source_domain_index}{target_domain_index}{randseed}] = HDA_CDLPP_SP(trainS_features,trainS_labels,trainT_features,trainT_labels,test_features,test_labels,options);
            else
